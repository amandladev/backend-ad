...
class CheckMatches extends Command
{
    ...
    public function handle()
    {
        ...
        // $apuestas = Apuesta::where('estado', 0)->groupBy('match_id')->select('match_id', 'usuario_id')->get();
        $apuestas = Apuesta::where('estado', 0)->whereNotNull('party_size')->groupBy('match_id')->select('match_id', 'usuario_id', 'party_size')->get();
        foreach($apuestas as $apuesta){
            try {
                ...
                if($match !== null){
                   ...
                   // Apuesta::where('match_id', $apuesta->match_id)->where('estado', '0')->update(['estado'=>'2','fecha_finalizado'=>time(),'match_hero_id'=>$this->findHeroId($apuesta->usuario_id, $players)]);
                    }     //
                }
            } catch (\Exception $e) {
                Log::error($e);

...
class CheckMatchesDollarTest extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'matches_dollar_test:check';
    ...
    public function handle()
    {
        ...
        $apuestas = ApuestaTestDollar::where('estado', 0)->groupBy('match_id')->select('match_id')->get();
                ...
                if ($radiant_win == true) {
                    ...
                    foreach($winners as $winner){
                        ...
                        if($user !== null){
                            ...
                            $user_partida = $user->apuestas_test_dollar()->where('match_id', $apuesta->match_id)->first();

                            if($user_partida !== null){
                                // Aumentar saldo
                                $balanceRepo = new BalanceRepo();
                                $balanceRepo->setUsuario($user);
                                $balanceRepo->increase(($user_partida->monto * $user_partida->multiplicador), 'balance_test_dollar');
                    ...
                    // Validamos si el ganador esta en nuestra BD
                        if($usuario !== null){
                            // buscamos su partida (apuesta)
                            $user_partida = $usuario->apuestas_test_dollar()->where('match_id', $apuesta->match_id)->first();
                    ...
                    ApuestaTestDollar::where('match_id', $apuesta->match_id)->where('estado', '0')->update(['estado'=>'2','fecha_finalizado'=>time()]);
                }
            } catch (\Exception $e) {
                Log::error($e);
            }

...
class ProcesarApuestasDollar extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'apuestas_dollar:process';
    ...
    {
        $config = DB::table('0_config')->where('id', 9)->first();
        ...
        $apuestas = ApuestaDollar::where('estado', 0)->whereNull('match_id')->get();
        ...
        DB::table('0_config')->where('id', 9)->update(['valor'=>1]);
        ...
        foreach($apuestas as $apuesta){
            ...
            try {
                ...
                if($diff > 0 && $diff < $apuesta_intervalo && $filtered_matches[0]['lobby_type'] == 7) {
                    ...
                    $apuesta->save();
                } else if ($filtered_matches[0]['lobby_type'] != 7){
                    Log::info("La partida ha sido jugada en grupo, no individual");
                    $apuesta->match_id = $filtered_matches[0]['match_id'];
                    $apuesta->match_start_time = $filtered_matches[0]['start_time'];
                    $apuesta->match_hero_id = $key;
                    $apuesta->estado = '3';
        ...
        DB::table('0_config')->where('id', 9)->update(['valor'=>0]);
    }

    private function findMatch($usuario_id, $match_id){
        return ApuestaDollar::where('usuario_id', $usuario_id)->where('match_id', $match_id)->first();
    }
